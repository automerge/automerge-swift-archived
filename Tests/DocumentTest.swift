...
class DocumentTest: XCTestCase {
    ...
    func testPerformingChanges2() {
        ...
        let actor = ActorId()
        var doc = Document(Schema(bird: nil), options: .init(actorId: actor, backend: nil))
    ...
    func testPerformingChanges3() {
        ...
        var doc = Document(Schema(birds: nil), options: .init(backend: nil))
        let req = doc.change({ $0[\.birds, "birds"] = .init(wrens: 3) })
        let birds = doc.getObjectId(\.birds, "birds")
        ...
        XCTAssertEqual(req, Request(requestType: .change, message: "", time: req!.time, actor: doc.actor.actorId, seq: 1, version: 0, ops: [
            Op(action: .makeMap, obj: ROOT_ID, key: "birds", insert: false, child: birds),
            Op(action: .set, obj: birds!, key: "wrens", insert: false, value: .int(3))
        ], undoable: true))
    }
    ...
    func testPerformingChanges4() {
        ...
        var doc1 = Document(Schema(birds: nil), options: .init(backend: nil))
        ...
        XCTAssertEqual(req, Request(requestType: .change, message: "", time: req!.time, actor: doc1.actor.actorId, seq: 2, version: 0, ops: [
    ...
    func testPerformingChanges5() {
        ...
        let actor = ActorId()
        let doc1 = Document(Schema(magpies: 2, sparrows: 15), options: .init(actorId: actor, backend: nil))
        var doc2 = doc1
        ...
        XCTAssertEqual(req, Request(requestType: .change, message: "", time: req!.time, actor: actor.actorId, seq: 2, version: 0, ops: [
    ...
    func testPerformingChanges6() {
        ...
        let birds = doc1.getObjectId(\.birds, "birds")
        ...
        XCTAssertEqual(req, Request(requestType: .change, message: "", time: req!.time, actor: doc1.actor.actorId, seq: 1, version: 0, ops: [
            Op(action: .makeList, obj: ROOT_ID, key: "birds", insert: false, child: birds),
            Op(action: .set, obj: birds!, key: 0, insert: true, value: .string("chaffinch"))
        ], undoable: true))
    }
    ...
    func testPerformingChanges8() {
        ...
        var doc1 = Document(Schema(birds: []), options: .init(backend: nil))
        ...
        XCTAssertEqual(req, Request(requestType: .change, message: "", time: req!.time, actor: doc2.actor.actorId, seq: 3, version: 0, ops: [
    ...
    func testPerformingChanges9() {
        ...
        var doc1 = Document(Schema(now: nil), options: .init(backend: nil))
    ...
    func testBackendConcurrency1() {
        ...
        var doc1 =  Document(Schema(blackbirds: nil, partridges: nil), options: .init(actorId: local ,backend: nil))
